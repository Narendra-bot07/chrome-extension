...
> **2026-08-09: JD extraction's four Pro workers bypass this tier entirely.** Since the [3.17.0](CHANGELOG.md) rewrite ([JD_EXTRACTION_ENGINE_DOCUMENTATION.md](JD_EXTRACTION_ENGINE_DOCUMENTATION.md) §8.18), `extraction_agent`'s Role/Skills/Responsibilities/Requirements workers call `get_llm(...).with_structured_output(...).invoke(...)` directly — none of them go through `llm_cache.execute_with_cache`, so there is no per-LLM-call fingerprint cache for JD extraction's primary path anymore (the old `jd_agent_extraction`/`jd_agent_extraction_facts`/`_content` task rows in the table above no longer accumulate). The only remaining protection against redundant Pro calls for identical evidence is the URL-level result cache in §8 below, which caches the whole pipeline's *final* response, not individual worker calls. `repair_agent` (`task="jd_agent_repair"`) still uses this tier, but is currently unreachable from the graph (§8.20 of the extraction doc) — its cache entries, if any exist from before the routing change, are dead weight.
...
## 9. Short-TTL Resume Lookup Cache (`/api/compare`, added 2026-08-09)

`api_compare` (`app/routers/api.py`) falls back to a Postgres lookup (`ResumeRepository.get_by_id`/`get_active`) whenever the caller didn't send a populated resume in the request body. In a normal tailoring session this fallback fires repeatedly in a short window — a user reviews a suggestion, calls compare again with a tweak, reviews again — each one re-fetching the identical resume row.

- **Key**: `resume_lookup:v1:{user_id}:{resume_id or 'active'}`, via `redis_cache` (the same raw transport used by §8, not `LLMCacheService`).
- **TTL**: 90 seconds — deliberately short, not the 24h/7d TTLs used elsewhere in this document. Resume content can be mutated from roughly 10 different write endpoints (`/layout`, `/rename`, `/parse`, `/recover-source`, `/versions/*`, `/activate`, ...); rather than wiring cache invalidation into every one of them, this tier accepts a bounded staleness window instead. **If you add a new resume-mutating endpoint, this cache is not automatically invalidated by it** — the 90s TTL is the only safety net today.
- **Not used when the caller already sent a populated resume** (`request.resume` has `experience`/`education`) — that's the common case for the extension's primary tailoring flow; this cache only helps the fallback path.
- Separate and unrelated to `ats_repo.get_cached_analysis` (a full comparison-result cache keyed by `resume_id`+`jd_id`+engine version, in Postgres via `ATSRepository`, not Redis) — that one can already short-circuit an entire `/api/compare` call including this resume lookup; this tier only helps when that full-result cache misses (different JD, different `selected_sections`, etc.) but the resume itself hasn't changed.
