...
- **Current Status**: Partially resolved (2026-08-03):
  ...
  - **(2026-08-04)** Separately confirmed: `core/database.py`'s pool `maxconn` was `50`, but Supabase's own pooler for this project's compute tier only grants **15** real connections total — the app's pool ceiling was never actually reachable, meaning checkouts were failing against Supabase's own limit before ever hitting psycopg2's. Reduced `maxconn` to `12` (see [CHANGELOG.md](CHANGELOG.md) 3.7.8). This makes the remaining hold-time fixes below *more* urgent, not less — with only 12-15 real connections available project-wide, every endpoint that still holds one during an LLM/Playwright call reduces effective concurrent capacity by a proportionally larger amount than it would on a higher tier.
