...
This pattern (now `RESOLVED` as of 2026-08-04, see [KNOWN_ISSUES.md](KNOWN_ISSUES.md) ISSUE-005) is applied across every previously-flagged endpoint: `api_cover_letter` and `api_generate_cover_letter_draft` (`app/routers/api.py`) use the plain `_db_context()` above; `tailor_resume` (`api/v1/tailoring.py`) and `download_pdf` use `api/dependencies.py::user_scoped_db_context(user_id)` — the same pattern but also sets the `request.jwt.claim.sub` RLS session variable, matching `get_user_db_connection`'s behavior, so it's the correct choice for any route whose repos rely on Postgres RLS. `TailoringService` (`services/resume/tailoring_service.py`) was split into `load_context()` / `compute_tailored_resume()` (pure computation, no repo access) / `persist_result()` so the route can open a connection around each DB phase independently, with nothing held during the LLM call.

For services whose DB access is interleaved *throughout* a multi-step pipeline rather than bookended at the start/end (e.g. `PostgresCheckpointStore` in `services/workflow/checkpoints.py`, which writes a checkpoint after every node of the resume-intelligence pipeline), the short-lived-connection pattern doesn't fit a simple before/after split. Instead, the class takes a **connection factory** (a zero-arg callable returning a context manager, e.g. `lambda: user_scoped_db_context(user_id)`) rather than one bound connection — since every method was already a fully self-contained transaction, each call now opens and closes its own short-lived connection instead of one connection sitting open (and idle) for the pipeline's entire duration. `build_selected_resume_intelligence` / `confirm_selected_resume_intelligence` (`api/v1/resume.py`) use this. `services/workflow/runtime.py` and `api/v1/workflows.py` were updated to match the new constructor signature, though those 5 routes intentionally keep their prior request-scoped connection behavior (wrapped in `contextlib.nullcontext`) since they weren't part of this fix's scope.
