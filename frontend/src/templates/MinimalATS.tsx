...
export default function MinimalATS({ resume }) {
  ...
  return (
    <div className="font-mono p-10 max-w-4xl mx-auto bg-white text-black" style={{ width: '8.5in' }}>
          ...
          <div className="space-y-4">
            {awards.map((award, i) => (
              <div key={i} className="text-sm ml-2">
                <span className="font-bold">{award.title}</span> {award.issuer && <span>- {award.issuer}</span>}
              </div>
            ))}
          </div>
        </section>
      ...
      {volunteer_experience && volunteer_experience.length > 0 && (
        <section className="mb-6" data-section="volunteer">
          <h2 className="text-sm font-bold uppercase mb-4">Volunteer</h2>
          <div className="space-y-4">
