...
function AuthLoadingContent({ mode }) {
  ...
  return <span className="public-auth-loading" role="status" aria-live="polite">
    <Loader2 className="public-auth-spinner" size={17} aria-hidden="true" />
    <span>{label}</span>
    <span className="public-auth-loading-dots" aria-hidden="true"><i /><i /><i /></span>
  </span>;
}
      ...
      <motion.div key={mode} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduced ? .08 : .16 }}>
        ...
        : <>
          ...
          <form onSubmit={submit}>
            ...
            <button className="public-auth-submit" disabled={loading} aria-busy={loading || undefined}>
              {loading
                ? <AuthLoadingContent mode={mode} />
                : <>{mode === 'register' ? 'Create Account' : mode === 'forgot_password' ? 'Send Reset Link' : 'Sign In'} <Zap size={15} /></>}
            </button>
          </form>
